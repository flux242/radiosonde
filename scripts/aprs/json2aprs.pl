...
#        my $bearingstr = defined $bearing ? sprintf(" B=%.0f°", $bearing) : "";
        ...
        my $str = sprintf("$mycallsign-15>APRS,TCPIP*:;%-9s*%06dh%s/%sO%03d/%03d/A=%06d!w%s%s!Clb=%.1fm/s%s%s%s%s Type=%s%s%s%s%s%s %s",
                          $callsign, $hms, $lat, $lon, $course, $speed, $alt, base91_from_decimal($latdoa), base91_from_decimal($londoa), $climb, $pressurestr, $tempstr, $humidstr, $freqstr, $type, $bktstr, $satstr, $battstr, $framestr, $otg,  $comment);
        print $fpo "$str\n";
